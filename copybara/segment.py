...
def segment_copy_number(outdir, smoothened_cn_path, min_segment_size, shuffles, p_seg, p_val, quantile, quantile_low_cov, min_coverage, coverage, threads):
    ...
    if round(coverage,2) <= min_coverage:
        quantile = quantile_low_cov
        print(f"    Low coverage ({coverage}x < {min_coverage}x). Segments merged with qunatile {quantile_low_cov}")
    if quantile == 0:
